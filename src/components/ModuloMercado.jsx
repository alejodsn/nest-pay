      ...
      <div className="px-2">
        ...
        <div className="mb-8">
          <div className="flex justify-between text-sm mb-3">
            <span className="text-text-main font-sans text-sm font-semibold">Progreso del Presupuesto</span>
            <span className="font-space font-semibold text-text-main tabular-nums">{porcentajeGastado.toFixed(1)}%</span>
          </div>
          <div className="h-2.5 w-full bg-surface-hover border border-border rounded-full overflow-hidden relative">
            <div 
              className={`h-full ${barraColor} transition-all duration-500`}
              style={{ width: `${Math.min(porcentajeGastado, 100)}%` }}
            ></div>
            <div 
              className="absolute top-0 bottom-0 w-0.5 bg-surface-hover0 z-10"
          ...
          <div className="flex justify-between mt-4">
            <div>
              ...
              <p className="text-xl font-space font-semibold text-text-main tabular-nums">{formatter.format(totalGastado)}</p>
            </div>
        ...
        {isAdding && (
          <form onSubmit={handleAddTicket} className="bg-surface-hover p-5 rounded-2xl border border-border mb-8 grid grid-cols-1 md:grid-cols-4 gap-4 items-end transition-all">
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  type="date" 
                  required
                  className="w-full pl-9 pr-3 py-2 rounded-xl border border-border bg-transparent text-text-main focus:ring-[#10B981] focus:border-[#10B981] text-sm font-medium outline-none"
                  value={fecha}
            ...
            <div>
              ...
              <input 
                ...
                required
                placeholder="0"
                className="w-full px-3 py-2 rounded-xl border border-border bg-transparent focus:ring-[#10B981] focus:border-[#10B981] text-right font-space font-semibold text-text-main tabular-nums outline-none"
                value={valor}
            ...
            <div className="flex gap-2 h-10">
              ...
              <button type="button" onClick={() => setIsAdding(false)} className="px-4 bg-surface-hover text-text-main rounded-xl hover:bg-surface-hover transition-colors font-medium text-sm outline-none border border-border">
                X
              </button>
            </div>
          </form>
        ...
        {/* Lista de Tickets */}
        <div>
          <h3 className="text-[11px] font-medium text-text-muted uppercase tracking-wider mb-4 border-b border-border pb-2">Historial de Compras</h3>
          <div className="space-y-1">
            {tickets.map(ticket => (
              <div key={ticket.id} className="flex justify-between items-center py-3 px-2 hover:bg-surface-hover rounded-xl border border-transparent transition-colors duration-150">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-surface-hover border border-border rounded-full flex items-center justify-center text-text-muted">
                    <ShoppingCart className="w-4 h-4" />
                  </div>
                  <div>
                    <p className="text-text-main font-sans text-sm font-semibold">{ticket.establecimiento}</p>
                    <p className="text-xs text-text-muted">{ticket.fecha}</p>
                  </div>
                </div>
                <div className="font-space font-semibold text-text-main tabular-nums">
