...
export default function TablaGastos({ mesId, perfil, datos, isAlejandro, configuracion }) {
  ...
  const FilaGasto = ({ item, valorCalculado, q1_pagado, q2_pagado, onToggleQ1, onToggleQ2, onDelete, onEdit, isAuto, isReserva }) => {
    if (editingId === item.id) {
      return (
        <tr className="bg-surface-hover border-b border-border">
          ...
          <td className="py-3">
            <input type="number" className="w-full border border-border bg-transparent text-text-main rounded p-1 text-sm text-right tabular-nums font-space font-semibold tracking-tight outline-none focus:outline-none focus:ring-0 focus:ring-offset-0 transition-all" value={editValor} onChange={(e) => setEditValor(e.target.value)} />
          </td>
          <td className="py-3 text-center text-text-muted text-xs">Calc...</td>
          <td className="py-3 text-center text-text-muted text-xs">Calc...</td>
          <td className="py-3">
            <div className="flex items-center justify-end gap-3">
              <label className="flex items-center gap-1 text-[11px] font-medium text-text-muted cursor-pointer">
                <input 
                  type="checkbox" 
                  className="cursor-pointer accent-emerald-500 rounded outline-none w-4 h-4 border border-border bg-surface-hover transition-all" 
                  checked={editMensual} 
    ...
    return (
      <tr className="hover:bg-surface-hover transition-colors duration-150 border-b border-border">
        <td className="py-3 text-text-main font-sans text-sm">
          {item.nombre}
        ...
        <td className="py-3 text-center">
          <label className="inline-flex items-center gap-2 cursor-pointer group">
            <input type="checkbox" checked={q1_pagado} onChange={onToggleQ1} className="cursor-pointer accent-emerald-500 rounded outline-none w-4 h-4 border border-border bg-surface-hover transition-all" />
            <span className={`transition-all font-space font-semibold tabular-nums ${q1_pagado ? 'line-through opacity-50 text-[#10B981]' : 'text-text-muted'}`}>
        ...
        <td className="py-3 text-center">
          <label className="inline-flex items-center gap-2 cursor-pointer group">
            <input type="checkbox" checked={q2_pagado} onChange={onToggleQ2} className="cursor-pointer accent-emerald-500 rounded outline-none w-4 h-4 border border-border bg-surface-hover transition-all" />
            <span className={`transition-all font-space font-semibold tabular-nums ${q2_pagado ? 'line-through opacity-50 text-[#10B981]' : 'text-text-muted'}`}>
              {formatter.format(item.valorQ2 || (item.valor / 2))}
            </span>
          </label>
        </td>
        <td className="py-3">
          {isAuto || isReserva ? <span className="text-xs text-text-muted italic block text-right font-medium">Auto</span> : (
  ...
  return (
    <>
      <NotchedBentoCard 
        title={<span className="pr-24 md:pr-0 block">Gastos Fijos & Obligaciones</span>}
        action={
          <button 
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="text-sm">
            ...
            <tr className="border-b border-border">
              <td colSpan="5" className="pt-6 pb-2 text-[11px] font-bold text-text-muted uppercase tracking-wider flex items-center gap-2"><Info className="w-3.5 h-3.5" /> Obligaciones de Ley</td>
            </tr>
            {inmutables.map(item => (
              <FilaGasto
            ...
            {/* SECCIÓN 2: FIJOS */}
            <tr className="border-b border-border">
              <td colSpan="5" className="pt-6 pb-2 text-[11px] font-bold text-text-muted uppercase tracking-wider flex items-center gap-2"><Info className="w-3.5 h-3.5" /> Gastos Fijos</td>
            </tr>
            ...
            {/* SECCIÓN 3: VARIABLES */}
            <tr className="border-b border-border">
              <td colSpan="5" className="pt-6 pb-2 text-[11px] font-bold text-text-muted uppercase tracking-wider flex items-center gap-2"><Info className="w-3.5 h-3.5" /> Gastos Variables (Mes en curso)</td>
            </tr>
            ...
            {isAdding && (
              <tr className="bg-surface-hover border-b border-border">
                <td className="py-3"><input type="text" placeholder="Ej. Regalo" className="w-full border border-border bg-transparent text-text-main rounded p-1 text-sm outline-none focus:outline-none focus:ring-0 focus:ring-offset-0 transition-all" value={nuevoNombre} onChange={(e) => setNuevoNombre(e.target.value)} /></td>
                <td className="py-3">
                  <input type="number" placeholder="Valor" className="w-full border border-border bg-transparent text-text-main rounded p-1 text-sm text-right font-space font-semibold tabular-nums outline-none focus:outline-none focus:ring-0 focus:ring-offset-0 transition-all" value={nuevoValor} onChange={(e) => setNuevoValor(e.target.value)} />
                </td>
                <td className="py-3 text-center text-text-muted text-xs">Calc...</td>
                <td className="py-3 text-center text-text-muted text-xs">Calc...</td>
                <td className="py-3">
            ...
            {!gastosVariables.length && !isAdding && (
              <tr><td colSpan="5" className="py-8 text-center text-text-muted italic border-b border-border">No hay imprevistos registrados este mes.</td></tr>
            )}

            {/* SECCIÓN 4: RESERVA */}
            <tr className="border-b border-border">
              <td colSpan="5" className="pt-6 pb-2 text-[11px] font-bold text-text-muted uppercase tracking-wider flex items-center gap-2"><Info className="w-3.5 h-3.5" /> Ahorro</td>
            </tr>
            <FilaGasto
              item={reserva}
